...
- 进度反馈只是反馈点，不是暂停点；progress < 100% 且未进入终止状态时，runner 必须继续工作。
- 主对话被新消息打断，不等于旧任务结束。
...
- `scripts/studio_completion_suggest.py`：从 dispatch/history/artifacts/logs 自动部分填充 completion_evidence 建议值
