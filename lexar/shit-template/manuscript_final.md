...
## 摘要
基于东亚五地公开社媒、论坛与群聊空间中的去标识化 AI 采纳叙事样本（清洗后 N=986），本文考察“表演性采纳”与短期留存之间的关系。我们以 7 日留存（DR7）为核心结果变量，构建 Logit 模型，并以弃用风险模型进行稳健性检验。结果显示：接入展示强度（AS）与留存显著负相关（OR=0.662，p<0.001），学习成本（LearningCost）显著提高弃用风险（HR=1.21，p<0.01），而任务清晰度（TaskClarity）是最稳定的正向预测因子（OR=1.681，p<0.001）。研究表明，问题未必首先出在“用户不够努力”或“工具还不够聪明”，而更可能出在平台展示激励、身份压力与学习摩擦之间的结构性失配。就平台治理与组织实践而言，与其奖励“已接入”的姿态性成功，不如奖励“仍能持续交付”的复盘性成功。
...
## 1 数据与样本
- 样本来源：东亚五地公开社媒/论坛/群聊中的 AI 采纳叙事文本（去标识化）。之所以选取这类材料，是因为它们最擅长在工具真正融入工作流之前，先制造一种“融入工作流的社会印象”。
- 初始样本：N=1,204
- 清洗规则：
  1) 去重（文本相似度 > 0.92）
  2) 删除缺失关键时间戳样本
...
机制上，研究支持“平台展示激励挤压学习投入”的解释路径：用户在首 48 小时将时间预算投向可见成果（截图、转发、身份表达），而非流程打磨与异常修复，导致摩擦发生后快速弃用。简言之，许多样本并不是先学会了工具，再学会了展示；而是先学会了展示自己已经学会，然后在真正需要解决报错、接口、流程与脏活时礼貌离场。

## 9 结论与启示
东亚 AI 工具热潮并非“用户不用功”或“工具不够强”的单因果问题，而更像是平台机制、身份压力与学习成本共同塑造的结构性失配。若将“发出接入信号”误认作“形成生产力”，组织就很容易把热情误记为能力，把展示误记为落地，把短暂在线误记为稳定采用。对平台与组织的实际启示至少有三点：
...
换句话说，决定生产率的往往不是“会不会发一张接入截图”，而是“在热搜退场、群聊安静、报错弹出之后，能不能继续把任务做完”。在这一意义上，真正稀缺的并不是接入本身，而是接入之后仍愿意面对摩擦的人。
